...
def mut_rev_cosine_dist(individual, pset, mapping, max_depth, model, distance, stats):
...
def mut_rev_euclid_dist(individual, pset, mapping, max_depth, model, distance, stats):
...
def de_mut(individual, pset, mapping, max_depth, model, population, stats, F=0.5, CR=0.5):
    mutated = None
    trials = 0
    stats["called"] += 1
    while mutated is None and trials < 3:
